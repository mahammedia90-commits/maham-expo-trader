/**
 * BookingGuard — Modal that blocks booking if KYC not verified — Fully translated
 */
...
interface BookingGuardProps { isOpen: boolean; onClose: () => void; onProceedToKYC: () => void; }
...
export default function BookingGuard({ isOpen, onClose, onProceedToKYC }: BookingGuardProps) {
  ...
  const steps = [
    { label: t("guard.accountRegistration"), done: true, icon: CheckCircle2 },
    { label: t("guard.uploadDocuments"), done: docsUploaded, icon: FileText },
    { label: t("guard.identityVerification"), done: kycStatus === "verified", icon: ShieldCheck },
    { label: t("guard.accountVerification"), done: accountVerified, icon: CheckCircle2 },
  ];
      ...
      <div className="w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl p-4 sm:p-6 shadow-2xl max-h-[92vh] overflow-y-auto" style={{ background: "var(--modal-bg)", border: "1px solid var(--glass-border)", paddingBottom: "max(env(safe-area-inset-bottom, 0px), 16px)" }}>
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold" style={{ color: "var(--text-primary)" }}>{t("guard.mustVerifyFirst")}</h3>
              <p className="text-[11px]" style={{ color: "var(--text-muted)" }}>{t("guard.completeVerification")}</p>
            </div>
          </div>
        ...
        <div className="rounded-xl p-4 mb-5" style={{ background: "var(--modal-inner-bg)", border: "1px solid var(--glass-border)" }}>
          <p className="text-sm leading-relaxed" style={{ color: "var(--text-secondary)" }}>
            {t("guard.verificationMessage")}
          </p>
        </div>

        <div className="mb-5">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium" style={{ color: "var(--text-secondary)" }}>{t("guard.verificationProgress")}</span>
          ...
          <div className="h-2 rounded-full overflow-hidden" style={{ background: "var(--glass-bg)" }}>
            <div className="h-full rounded-full transition-all duration-500" style={{ width: `${progress}%`, background: `linear-gradient(90deg, var(--gold-accent), var(--gold-light))` }} />
          </div>
        </div>
          ...
          {steps.map((step, i) => (
            <div key={i} className="flex items-center gap-3 p-3 rounded-xl transition-all"
              ...
              <div className="flex-1">
                <span className="text-xs font-medium" style={{ color: step.done ? "var(--gold-accent)" : "var(--text-secondary)" }}>{step.label}</span>
              </div>
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-3 rounded-xl text-sm font-medium transition-all flex items-center justify-center gap-2"
